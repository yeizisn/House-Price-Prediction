...
# Load the data.
train_df = pd.read_csv("Subtrain.csv")
test_df = pd.read_csv("Subtest.csv")
...
'''
#get the columns that just exist in train and drop them
#get the columns that just exist in test and drop them

print("Training set size:", train_df_munged.shape)
print("Test set size:", test_df_munged.shape)
list(train_df_munged)


train_only = list(set(train_df_munged)-set(test_df_munged))
test_only = list(set(test_df_munged)-set(train_df_munged))
print "train_only:"
print train_only

print "test_only"
print test_only
train_df_munged.drop(train_only, axis=1, inplace=True)
test_df_munged.drop(test_only, axis=1, inplace=True)
...
y_subtest = test_df['SalePrice']
print("Lasso score on test set: ", rmse(y_subtest, np.exp(y_pred_lasso)))
print("XGBoost score on test set: ", rmse(y_subtest, np.exp(y_pred_xgb)))

print("Lasso R_2 Score", r2_score(y_subtest,np.exp(y_pred_lasso),multioutput="variance_weighted"))
print("XGBoost R_2 Score", r2_score(y_subtest,np.exp(y_pred_xgb),multioutput="variance_weighted"))

Price_lasso  = pd.DataFrame(np.exp(y_pred_lasso),index=test_df['Id'],columns=['SalePrice'])
Price_xgb = pd.DataFrame(np.exp(y_pred_xgb),index=test_df['Id'],columns=['SalePrice'])

Price_lasso.to_csv('Lasso_output.csv',header=True, index_label='Id')
Price_xgb.to_csv('xgb_output.csv',header=True,index_label='Id')

y_pred = (y_pred_xgb + y_pred_lasso) / 2
y_pred = np.exp(y_pred)
print("XGBoost score on test set: ", rmse(y_subtest, y_pred))

print("XGBoost r2_score on test set: ", r2_score(y_subtest, y_pred,multioutput="variance_weighted"))

pred_df = pd.DataFrame(y_pred, index=test_df["Id"], columns=["SalePrice"])
pred_df.to_csv('output2.csv', header=True, index_label='Id')
